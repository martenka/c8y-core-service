...
import { File, FileSchema, Sensor, VisibilityState } from '../../../models';
...
describe('Files integration test', () => {
  ...
  it('gets a specific file', async () => {
    ...
    await sensorModel.create(sensor1);
  ...
  it('gets current file link', async () => {
    ...
    expect(output.body).toMatchObject<FileLink>({
      ...
      fileName: 'File2',
      url: testingConfigService.minioConfig.url + '/test_bucket/File2.csv',
    });
  });

  it('does not get file link when file is unpublished and user is not admin', async () => {
  ...
  it('checks files for upload readiness', async () => {
  ...
  it('changes file visibility state', async () => {
  ...
  it('deletes one file', async () => {
    ...
    expect((await sensorModel.findById(sensor1Id))._id.toString()).toEqual(
      '647c5cbb41c4e44da374e62b',
    );
    expect(await fileModel.findById(fileStub._id)).toBeNull();
    expect(sendMessageSpy).toHaveBeenCalledWith<SendMessageParams>(
      ExchangeTypes.GENERAL,
  ...
  it('deletes many files', async () => {
    ...
    const output = await request(app.getHttpServer())
      .post('/files/delete')
      .send({ items: ['647c5ec604b53a5cc9ce4e80', '647c5f16e12f0434c9a4dbd6'] })
      .set('Authorization', `Bearer ${testUser.userToken}`);
  ...
  it('searches files from time frame', async () => {
    const testUser = getTestUser(
  ...
  it('searches files by sensor valueFragmentType (data type) sorting newer files first', async () => {
    const testUser = getTestUser(
